...
def generate_candidates(usable_poly, fallback_poly, column_polys, bbox, presets,
    ...
    # Same relaxed AREA ratio layout_engine._place_auditoriums_inner's greedy
    # custom-fit backtracking uses (see that module's own comment for the
    # full reasoning and the real, measured case it fixes): a custom-fit
    # room already carries its own "review before finalizing" note, so it
    # can tolerate a bit more real structural column presence than a preset
    # match. The POSITION gate (edge_tolerance_ft) is NOT relaxed — a column
    # stranded in the interior of a large custom-fit candidate (not near any
    # wall) must still be rejected, same real client-reported defect
    # (a custom-fit room silently swallowing every column scattered through
    # its interior) that motivated enforcing this uniformly.
        ...
        for rx, ry, rw, rh in rects:
            ...
            if cw * ch >= min_preset_area_sqft:
                for ax, ay in ((rx, ry), (rx + rw - cw, ry + rh - ch)):
                    ...
                    if used_fallback and not column_enclosure.enclosure_ok(ax, ay, cw, ch, column_polys, custom_fit_column_cap, aud_edge_tolerance_ft):
                        continue
